...
class DBMSEvaluator:
    ...
    @monitor()
    def neo4jee_col_1015074_SBC_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_1015074_SBC_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1015074_SBC_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1015074_SBC_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1015074_SBC_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_1015074_SBC_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_1015074_SBC_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_ppi_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_ppi_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_ppi_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_ppi_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_ppi_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_ppi_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_ppi_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_1000_5_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_1000_5_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_5_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_5_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_5_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_1000_5_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_1000_5_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_1000_10_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_1000_10_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_10_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_10_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_10_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_1000_10_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_1000_10_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_1000_20_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_1000_20_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_20_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_20_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_20_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_1000_20_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_1000_20_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_1000_SF_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_1000_SF_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_SF_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_SF_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000_SF_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_1000_SF_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_1000_SF_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_10000_5_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_10000_5_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_5_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_5_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_5_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_10000_5_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_10000_5_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_10000_10_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_10000_10_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_10_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_10_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_10_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_10000_10_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_10000_10_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_10000_20_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_10000_20_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_20_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_20_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_20_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_10000_20_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_10000_20_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_10000_SF_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_10000_SF_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_SF_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_SF_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_10000_SF_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_10000_SF_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_10000_SF_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_100000_5_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_100000_5_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_5_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_5_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_5_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_100000_5_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_100000_5_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_100000_10_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_100000_10_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_10_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_10_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_10_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_100000_10_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_100000_10_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_100000_20_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_100000_20_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_20_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_20_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_20_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_100000_20_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_100000_20_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_100000_SF_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_100000_SF_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_SF_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_SF_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_100000_SF_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_100000_SF_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_100000_SF_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_1000000_5_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_1000000_5_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000000_5_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000000_5_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000000_5_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_1000000_5_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_1000000_5_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_col_1000000_SF_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_col_1000000_SF_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000000_SF_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000000_SF_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_col_1000000_SF_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_col_1000000_SF_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_col_1000000_SF_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_1015074_SBC_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_1015074_SBC_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1015074_SBC_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1015074_SBC_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1015074_SBC_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_1015074_SBC_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_1015074_SBC_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_ppi_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_ppi_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_ppi_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_ppi_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_ppi_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_ppi_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_ppi_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_1000_5_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_1000_5_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_5_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_5_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_5_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_1000_5_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_1000_5_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_1000_10_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_1000_10_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_10_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_10_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_10_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_1000_10_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_1000_10_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_1000_20_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_1000_20_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_20_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_20_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_20_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_1000_20_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_1000_20_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_1000_SF_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_1000_SF_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_SF_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_SF_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000_SF_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_1000_SF_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_1000_SF_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_10000_5_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_10000_5_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_5_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_5_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_5_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_10000_5_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_10000_5_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_10000_10_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_10000_10_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_10_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_10_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_10_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_10000_10_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_10000_10_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_10000_20_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_10000_20_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_20_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_20_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_20_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_10000_20_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_10000_20_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_10000_SF_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_10000_SF_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_SF_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_SF_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_10000_SF_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_10000_SF_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_10000_SF_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_100000_5_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_100000_5_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_5_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_5_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_5_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_100000_5_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_100000_5_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_100000_10_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_100000_10_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_10_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_10_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_10_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_100000_10_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_100000_10_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_100000_20_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_100000_20_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_20_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_20_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_20_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_100000_20_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_100000_20_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_100000_SF_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_100000_SF_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_SF_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_SF_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_100000_SF_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_100000_SF_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_100000_SF_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_1000000_5_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_1000000_5_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000000_5_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000000_5_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000000_5_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_1000000_5_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_1000000_5_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def neo4jee_list_1000000_SF_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def neo4jee_list_1000000_SF_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000000_SF_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000000_SF_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def neo4jee_list_1000000_SF_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def neo4jee_list_1000000_SF_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def neo4jee_list_1000000_SF_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time

            
    @monitor()
    def mysql_col_1015074_SBC_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def mysql_col_1015074_SBC_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def mysql_col_1015074_SBC_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def mysql_col_1015074_SBC_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def mysql_col_1015074_SBC_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def mysql_col_1015074_SBC_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def mysql_col_1015074_SBC_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time
    ...
    @monitor()
    def mysql_list_1015074_SBC_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def mysql_list_1015074_SBC_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def mysql_list_1015074_SBC_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def mysql_list_1015074_SBC_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def mysql_list_1015074_SBC_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def mysql_list_1015074_SBC_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def mysql_list_1015074_SBC_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time
    ...
    @monitor()
    def postgres_col_1015074_SBC_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def postgres_col_1015074_SBC_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def postgres_col_1015074_SBC_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def postgres_col_1015074_SBC_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def postgres_col_1015074_SBC_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def postgres_col_1015074_SBC_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def postgres_col_1015074_SBC_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time
    ...
    @monitor()
    def postgres_list_1015074_SBC_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def postgres_list_1015074_SBC_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def postgres_list_1015074_SBC_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def postgres_list_1015074_SBC_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def postgres_list_1015074_SBC_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def postgres_list_1015074_SBC_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def postgres_list_1015074_SBC_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time
    ...
    @monitor()
    def sqlite_col_1015074_SBC_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def sqlite_col_1015074_SBC_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def sqlite_col_1015074_SBC_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def sqlite_col_1015074_SBC_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def sqlite_col_1015074_SBC_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def sqlite_col_1015074_SBC_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def sqlite_col_1015074_SBC_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time
    ...
    @monitor()
    def sqlite_list_1015074_SBC_create(self, crud_evaluator):
        create_time = crud_evaluator.create()
        return create_time

            
    @monitor()
    def sqlite_list_1015074_SBC_read_1(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def sqlite_list_1015074_SBC_read_2(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def sqlite_list_1015074_SBC_read_3(self, crud_evaluator, hops, assert_features_and_labels):
        read_time, test_time = crud_evaluator.read(hops, assert_ids = True ,assert_edge_index = True, assert_features = assert_features_and_labels, assert_labels = assert_features_and_labels)
        return read_time, test_time

                
    @monitor()
    def sqlite_list_1015074_SBC_update_nodes(self, crud_evaluator):
        update_node_time = crud_evaluator.update_nodes()
        return update_node_time

            
    @monitor()
    def sqlite_list_1015074_SBC_update_edges(self, crud_evaluator):
        update_edge_time = crud_evaluator.update_edges()
        return update_edge_time

            
    @monitor()
    def sqlite_list_1015074_SBC_delete(self, crud_evaluator):
        delete_time = crud_evaluator.delete()
        return delete_time
        ...
        for dataset_name in datasets:
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1015074_SBC" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_1015074_SBC_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_1015074_SBC_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_1015074_SBC_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_1015074_SBC_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_1015074_SBC_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_1015074_SBC_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "ppi" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_ppi_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_ppi_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_ppi_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_ppi_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_ppi_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_ppi_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000_5" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_5_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_5_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_5_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_1000_5_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_1000_5_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_1000_5_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000_10" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_10_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_10_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_10_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_1000_10_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_1000_10_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_1000_10_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000_20" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_20_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_20_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_20_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_1000_20_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_1000_20_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_1000_20_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000_SF" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_SF_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_SF_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000_SF_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_1000_SF_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_1000_SF_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_1000_SF_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "10000_5" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_5_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_5_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_5_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_10000_5_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_10000_5_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_10000_5_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "10000_10" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_10_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_10_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_10_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_10000_10_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_10000_10_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_10000_10_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "10000_20" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_20_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_20_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_20_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_10000_20_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_10000_20_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_10000_20_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "10000_SF" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_SF_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_SF_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_10000_SF_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_10000_SF_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_10000_SF_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_10000_SF_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "100000_5" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_5_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_5_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_5_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_100000_5_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_100000_5_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_100000_5_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "100000_10" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_10_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_10_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_10_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_100000_10_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_100000_10_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_100000_10_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "100000_20" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_20_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_20_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_20_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_100000_20_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_100000_20_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_100000_20_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "100000_SF" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_SF_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_SF_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_100000_SF_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_100000_SF_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_100000_SF_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_100000_SF_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000000_5" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000000_5_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000000_5_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000000_5_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_1000000_5_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_1000000_5_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_1000000_5_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000000_SF" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000000_SF_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000000_SF_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_col_1000000_SF_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_col_1000000_SF_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_col_1000000_SF_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_col_1000000_SF_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1015074_SBC" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_1015074_SBC_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_1015074_SBC_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_1015074_SBC_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_1015074_SBC_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_1015074_SBC_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_1015074_SBC_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "ppi" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_ppi_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_ppi_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_ppi_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_ppi_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_ppi_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_ppi_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000_5" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_5_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_5_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_5_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_1000_5_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_1000_5_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_1000_5_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000_10" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_10_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_10_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_10_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_1000_10_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_1000_10_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_1000_10_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000_20" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_20_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_20_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_20_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_1000_20_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_1000_20_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_1000_20_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000_SF" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_SF_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_SF_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000_SF_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_1000_SF_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_1000_SF_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_1000_SF_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "10000_5" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_5_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_5_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_5_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_10000_5_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_10000_5_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_10000_5_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "10000_10" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_10_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_10_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_10_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_10000_10_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_10000_10_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_10000_10_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "10000_20" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_20_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_20_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_20_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_10000_20_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_10000_20_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_10000_20_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "10000_SF" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_SF_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_SF_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_10000_SF_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_10000_SF_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_10000_SF_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_10000_SF_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "100000_5" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_5_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_5_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_5_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_100000_5_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_100000_5_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_100000_5_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "100000_10" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_10_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_10_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_10_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_100000_10_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_100000_10_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_100000_10_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "100000_20" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_20_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_20_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_20_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_100000_20_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_100000_20_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_100000_20_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "100000_SF" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_SF_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_SF_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_100000_SF_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_100000_SF_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_100000_SF_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_100000_SF_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000000_5" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000000_5_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000000_5_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000000_5_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_1000000_5_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_1000000_5_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_1000000_5_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1000000_SF" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000000_SF_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000000_SF_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.neo4jee_list_1000000_SF_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.neo4jee_list_1000000_SF_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.neo4jee_list_1000000_SF_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.neo4jee_list_1000000_SF_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1015074_SBC" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.mysql_col_1015074_SBC_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.mysql_col_1015074_SBC_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.mysql_col_1015074_SBC_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.mysql_col_1015074_SBC_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.mysql_col_1015074_SBC_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.mysql_col_1015074_SBC_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1015074_SBC" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.mysql_list_1015074_SBC_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.mysql_list_1015074_SBC_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.mysql_list_1015074_SBC_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.mysql_list_1015074_SBC_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.mysql_list_1015074_SBC_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.mysql_list_1015074_SBC_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1015074_SBC" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.postgres_col_1015074_SBC_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.postgres_col_1015074_SBC_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.postgres_col_1015074_SBC_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.postgres_col_1015074_SBC_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.postgres_col_1015074_SBC_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.postgres_col_1015074_SBC_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1015074_SBC" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.postgres_list_1015074_SBC_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.postgres_list_1015074_SBC_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.postgres_list_1015074_SBC_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.postgres_list_1015074_SBC_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.postgres_list_1015074_SBC_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.postgres_list_1015074_SBC_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "col" == self.Dbms_evaluator_class.file_suffix() and db_match and "1015074_SBC" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.sqlite_col_1015074_SBC_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.sqlite_col_1015074_SBC_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.sqlite_col_1015074_SBC_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.sqlite_col_1015074_SBC_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.sqlite_col_1015074_SBC_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.sqlite_col_1015074_SBC_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
            ...
            if "list" == self.Dbms_evaluator_class.file_suffix() and db_match and "1015074_SBC" == dataset_name:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '1']):
                    ...
                    read_time, test_time = self.sqlite_list_1015074_SBC_read_1(crud_evaluator, 1, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '1'].append(read_time)
                    read_times['1'] = read_time
                    read_times_mem['1'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '2']):
                    ...
                    read_time, test_time = self.sqlite_list_1015074_SBC_read_2(crud_evaluator, 2, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '2'].append(read_time)
                    read_times['2'] = read_time
                    read_times_mem['2'] = test_time
                else:
                ...
                if not is_stable(self.time_store[evaluation_dict_key]["read_" + '3']):
                    ...
                    read_time, test_time = self.sqlite_list_1015074_SBC_read_3(crud_evaluator, 3, assert_features_and_labels)
                    self.time_store[evaluation_dict_key]["read_" + '3'].append(read_time)
                    read_times['3'] = read_time
                    read_times_mem['3'] = test_time
                else:
                ...
                update_node_time, update_edge_time, delete_time = None, None, None
                if not is_stable(self.time_store[evaluation_dict_key]["update_nodes"]):
                    update_node_time = self.sqlite_list_1015074_SBC_update_nodes(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_nodes"].append(update_node_time)
                    self.is_update_nodes_stable = True
                if not is_stable(self.time_store[evaluation_dict_key]["update_edges"]):
                    update_edge_time = self.sqlite_list_1015074_SBC_update_edges(crud_evaluator)
                    self.time_store[evaluation_dict_key]["update_edges"].append(update_edge_time)
                    self.is_update_edges_stable = True
                if not everything_is_stable:
                    delete_time = self.sqlite_list_1015074_SBC_delete(crud_evaluator)
                    self.time_store[evaluation_dict_key]["delete"].append(delete_time)
                    self.is_delete_stable = is_stable(self.time_store[evaluation_dict_key]["delete"])
