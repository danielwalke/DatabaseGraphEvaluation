...
class Neo4jCol(Neo4jQuery, Neo4jConnector):
    ...
    def create(self):
        start = time.time()
        self.session.run(self.create_nodes_columns_query, file=f"file:///{self.X_and_y_file_name}")
    ...
    def read(self, seed_node_id, hops):
        start = time.time()
        results = self.session.run(self.read_subgraph_columns_query_dict[hops], seed_node_id = seed_node_id)
    ...
    def update_nodes(self, node_id):
        start = time.time()
        self.session.run(self.update_nodes_columns_query, node_id=node_id)
